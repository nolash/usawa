...
def encrypt_seed(seed: bytes, passphrase: str, opslimit=None, memlimit=None) -> bytes:
    if opslimit is None:
...
def setup_wallet(cfg, wallet_dir=None):
    ...
    ops = int(cfg.get("WALLET_OPSLIMIT", 0)) or nacl.pwhash.OPSLIMIT_INTERACTIVE
    mem = int(cfg.get("WALLET_MEMLIMIT", 0)) or nacl.pwhash.MEMLIMIT_INTERACTIVE

    encrypted = encrypt_seed(random_bytes, passphrase, opslimit=ops, memlimit=mem)
    ...
    print(f"public key saved to: {publickey_path}")
    ...
    print(f"your 32-byte public key (hex): {pk.encode().hex()}")
...
def main():
    ...
    argp.add_argument(
        "-v", type=str, choices=["info", "debug", "warning", "error"], help="be verbose"
    )
